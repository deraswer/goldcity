...
const BridgePage = () => {
  ...
  // API mutations
  const [requestVerification, { isLoading: isVerifying }] =
    useRequestVerificationMutation();
  const [bridgeTokens, { isLoading: isBridging }] = useBridgeTokensMutation();

  // Query transfer status if we have a transferId
  const { data: transferStatus, isSuccess: transferStatusSuccess } =
    useGetTokenTransferStatusQuery(transferId, {
      skip: !transferId,
    ...
    const loadTokens = async () => {
      if (sourceChain === "polygon") {
        ...
        setAvailableTokens([
          {
            symbol: "GOLD",
            address: "0x8415b5f0ae583E8581673427C007c720Aa610706",
            name: "Gold Token",
          },
          {
            symbol: "OIL",
        ...
        setAvailableTokens([
          {
            symbol: "GOLD",
            address: "Gold11111111111111111111111111111111111111",
            name: "Gold Token",
          },
        ]);
      }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
          ...
          <p className="text-xs text-[var(--text-secondary)] mt-1">
            Your current connected wallet chain
          </p>
        </div>
  ...
  const renderStep2 = () => (
    <div>
      ...
      <p className="text-[var(--text-secondary)] mb-6">
        Choose the token and amount you want to bridge from {sourceChain} to{" "}
        {targetChain}.
      </p>
      ...
      <div className="flex space-x-4">
        ...
        <button
          ...
          disabled={
            isBridging || !selectedToken || !amount || parseFloat(amount) <= 0
          }
  ...
  const renderStep3 = () => (
    <div className="text-center">
      ...
      <p className="text-[var(--text-secondary)] mb-6">
        Your bridge transaction is being processed and will be completed
        shortly.
      </p>
              ...
              <div
                className={`w-8 h-8 flex items-center justify-center rounded-full ${
                  step >= 1
                    ...
                    : "bg-[var(--background-secondary)] text-[var(--text-secondary)] border border-[var(--border-color)]"
                }`}
              >
                1
              </div>
