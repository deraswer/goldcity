...
const DashboardPage = () => {
  ...
  const {
    ...
    skip: !isAuthenticated || !token, // Скипаем, если нет токена
  });

  // Fetch DID document if user has a DID
  const { data: didData } = useGetDIDQuery(profileData?.user?.did, {
    skip: !profileData?.user?.did || isLoading || isError,
  });

  // Fetch credentials for this DID
  const { data: credentialsData } = useGetCredentialsForSubjectQuery(
    profileData?.user?.did,
    {
      skip: !profileData?.user?.did || isLoading || isError,
    }
  );
  ...
  }, [didData, credentialsData, dispatch, user?.walletAddress]);

  // If user doesn't have a DID or profile data is not loaded due to 401
  if (isLoading) {
      ...
      <div className="container mx-auto p-4">
        <div className="dark-card glass-effect p-6 text-center">
          ...
          <p className="text-[var(--text-secondary)] mb-6">
            Failed to load profile. Please try again or reconnect your wallet.
          </p>
          <button
            onClick={() => {
              refetch();
  ...
  if (!did && !profileData?.user?.did && profileData?.success) {
    return (
      <div className="container mx-auto p-4">
        <div className="dark-card glass-effect p-6 text-center">
          <h1 className="text-2xl font-bold mb-4">
            Welcome to Identity Bridge
          </h1>
          <p className="text-[var(--text-secondary)] mb-6">
            You need to create a decentralized identity (DID) to get started.
          </p>
          <Link
            to="/profile/identity/create"
            className="dark-button inline-block"
          >
            Create Identity
          </Link>
        ...
        <div className="dark-card glass-effect p-6 md:col-span-2">
          ...
          <div className="mb-4">
            ...
            <p className="font-mono text-sm break-all text-[var(--text-primary)]">
              {did || profileData?.user?.did || "Not created"}
            </p>
          </div>
          <div className="mb-4">
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-[var(--text-secondary)]">
                  Your decentralized identity is active
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              {credentialsData.credentials.slice(0, 3).map((credential) => (
                <div
                  ...
                  className="border border-[var(--border-color)] rounded-lg p-4 bg-[var(--background-secondary)] hover:bg-[var(--hover-bg)] transition-colors"
                >
                  ...
                  <p className="text-sm text-[var(--text-secondary)] mt-1">
                    Issued by: {credential.issuer_did}
                  </p>
                  <p className="text-sm text-[var(--text-secondary)]">
                    Issued:{" "}
                  ...
                  <Link
                    to={`/profile/credentials/${credential.credential_hash}`}
