...
const WalletConnect = () => {
  ...
  const connectWallet = async () => {
    ...
    try {
      ...
      // Get the challenge
      const chainName = chainId === 80002 ? "polygon" : "polygon"; // Убедись, что chainName корректен
      ...
      if (verifyResponse.success) {
        // Set the credentials in state using only the token
        dispatch(
          setCredentials({
            user: { walletAddress }, // Используем walletAddress из текущего контекста
            token: verifyResponse.token,
          })
        );

        // Navigate to dashboard
        navigate("/profile/dashboard"); // Обновлен путь для соответствия ProfileRoutes
      }
    } catch (error) {
